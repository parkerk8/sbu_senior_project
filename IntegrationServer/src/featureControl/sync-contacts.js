...
async function initalSetupGoogleContacts (boardItems) { // makes new database.
  ...
  while (boardItemIndex < boardItems.length) {
    ...
    let columnValuesIndex = 0; const currentItem = boardItems[boardItemIndex]; const name = currentItem.name
    const arrName = name.split(' ', 2); const arrEmails = []; const arrPhoneNumber = []; const arrNotes = []; let itemID = ''

    if (doConfig) {
      const columnIdConfig = []
